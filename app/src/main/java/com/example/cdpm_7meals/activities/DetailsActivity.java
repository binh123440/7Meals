...
public class DetailsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        AddToCart = findViewById(R.id.AddToCardButton);
            ...
            public void onClick(View v) {
                ...
                for (ItemCart item : shoppingCart.getCartItems()){
                    if(item.getProduct().getId() == product.getId()){
                        shoppingCart.addItem(product,Integer.parseInt(price.getText().toString()));
                        Toast.makeText(DetailsActivity.this,"Sản phẩm đã có trong giỏ hàng +"+ tvTotal.getText().toString(), Toast.LENGTH_SHORT).show();
                        return;
                    }
                }
                shoppingCart.addItem(product,Integer.parseInt(tvTotal.getText().toString()));
            }
        });
